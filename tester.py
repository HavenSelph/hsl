...
def handle_test(compiler: str, path: Path, expected: Expected) -> Tuple[bool, str, Path]:
    process = run(
        [compiler, "--disable-context", "-c", str(path)],
        stdout=PIPE,
        stderr=PIPE
    )
...
def main():
    ...
    parser.add_argument(
        ...
        "--compiler",
        default="debug",
        help="Which compiler to use for testing. (default: debug)"
    )
    parser.add_argument(
        "files",
        nargs="?",
    ...
    if args.compiler != 'debug':
        run(["cargo", "build", "--quiet", f"--{args.compiler}"])
    else:
        run(["cargo", "build", "--quiet"])
    compiler_path = Path().cwd() / 'target' / args.compiler / 'hsl'
    if compiler_path.with_suffix('.exe').exists():
        print("[+] Assuming on Windows, .exe found")
        compiler_path = compiler_path.with_suffix('.exe')
    else:
        print("[+] Assuming on Linux, no .exe found")
    if not compiler_path.exists():
        print(f"[-] Interpreter {compiler_path} not found")
        exit(1)
    ...
    arguments = [
        (compiler_path, test_path, expected)
